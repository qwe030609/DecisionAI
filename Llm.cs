// ============================================================================
//  LLM 轉接器（Agents 模組的最底層）
//   - FakeLlm              離線、可重現；行為由呼叫端注入的 responder 決定
//   - AnthropicLlm         Anthropic Messages API（純 HttpClient）
...
public sealed record LlmRequest(string Role, string System, string User, double Temperature);
...
/// <summary>假模型：不會思考，只回傳呼叫端指定的合成輸出，讓整個控制流程、驗證器與引擎可以離線跑通。</summary>
public sealed class FakeLlm : ILlm
{
    private readonly Func<LlmRequest, Random, string> _respond;
    private readonly Random _rng;
    public string Name { get; }

    public FakeLlm(string name, int seed, Func<LlmRequest, Random, string> respond)
    {
        Name = name; _rng = new Random(seed); _respond = respond;
    }

    public Task<string> CompleteAsync(LlmRequest req, CancellationToken ct = default)
    {
        lock (_rng) return Task.FromResult(_respond(req, _rng));
    }
}
