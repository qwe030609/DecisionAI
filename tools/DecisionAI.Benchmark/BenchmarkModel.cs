// ============================================================================
//  Benchmark 案例模型 — 把「八維小數 profile」換成 Rev1 的事實欄位。
//  Rev1 §4.1 的硬性要求：ProblemFacts 只放事實與類別，不得出現 LLM 自估的小數。
...
public sealed record Expected(Disposition Disposition, ReasonCode Code = ReasonCode.None,
                              Strategy? Strategy = null, string? Workflow = null)
{
    public static Expected Abstain(ReasonCode code) => new(Disposition.Abstain, code);
    public static Expected Proceed(Strategy s, string wf) => new(Disposition.Proceed, ReasonCode.None, s, wf);
}

public sealed record HypSpec(string Statement, double Confidence, int[] For, int[] Against);
...
public sealed record BenchCase
{
    ...
    public string WhyRev1 { get; init; } = "";            // 這個判定的理由（顯示在 UI）
    public double Budget { get; init; } = 3.0;
}
