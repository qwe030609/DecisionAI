// ============================================================================
//  Agents — Registry（讀 PolicySnapshot，本身無狀態）+ Runner
//  Phase 1：無相關性估計，只有譜系欄位；異質性靠「同廠商 / 同家族」懲罰。
// ============================================================================
...
public sealed class AgentRegistry : IAgentRegistry
{
    ...
    /// <summary>分數 = Beta 後驗均值（來自 pin 住的 snapshot）+ 專精加分 − 同廠商 / 同家族懲罰。貪婪選 k 個。</summary>
    public IReadOnlyList<RegisteredAgent> Select(SelectionQuery q, PolicySnapshot policy)
    {
        ...
        var pool = _agents
            .Where(a => a.Spec.Roles.Contains(q.Role) && !banned.Contains(a.Spec.AgentId))
            .Where(a => q.Domain == "*" || a.Spec.Domains.Contains(q.Domain) || a.Spec.Domains.Contains("*"))
            .ToList();
